...
Scenario: Retailer Name and Retailer Id are prepopulated
	...
	When I select create a new instance of process 'B07 Retailer requested change to size, model or location of a meter performed by the Wholesaler'
	Then the retailer name field and retailer id field are not empty

Scenario: A new B7 service request can be created
	Given I am logged in as a SWWBS user 
	And I choose the 'Start Process' link in the left hand menu
	And I select create a new instance of process 'B07 Retailer requested change to size, model or location of a meter performed by the Wholesaler'
	...
	And I select 'B. Metering' and sub process 'B07 Retailer requested change to size, model or location ofa meter performed by the Wholesaler' 
	And the submitted process can be seen in the processes I started list

Scenario Outline: A wholesaler can reject a B7 process from a retailer using all reasons where the process can continue
	Given a retailer has created Process B request 'B07 Retailer requested change to size, model or location of a meter performed by the Wholesaler'
